...
using Soenneker.Granola.Runners.OpenApiClient.Utils.Abstract;
...
namespace Soenneker.Granola.Runners.OpenApiClient.Utils;
...
public sealed class FileOperationsUtil : IFileOperationsUtil
{
    ...
    public async ValueTask Process(CancellationToken cancellationToken = default)
    {
        ...
        string openApiDocumentUrl = _configuration["Granola:ClientGenerationUrl"] ?? "https://docs.granola.ai/api-reference/openapi.json";
        ...
        if (filePath == null)
            throw new InvalidOperationException("Granola OpenAPI document download failed.");
        ...
        await _kiotaUtil.Generate(fixedFilePath, "GranolaOpenApiClient", Constants.Library, gitDirectory, cancellationToken).NoSync();
    ...
    private async ValueTask BuildAndPush(string gitDirectory, CancellationToken cancellationToken)
    {
        ...
        if (!successful)
            throw new InvalidOperationException("The generated Granola OpenAPI client did not build successfully.");
